    ...
    return (
        <div>
            <div className="font-bold flex items-center justify-start text-primary gap-3">
                ...
                <h1 className="text-2xl tracking-wider">Form.</h1>
            </div>

            <section className="my-6">
                <div className={`flex items-center justify-${sidebarCollapsed ? "start" : "end"} w-full`}>
                    <button onClick={handleSidebarCollapseState} className="text-primary">
                ...
                <div className="mt-4 flex flex-col gap-6">
                    {StepData.map((el: StepType, idx) => {
                        return <Step key={idx} stepTitle={el.stepTitle} stepNumber={el.stepNumber} activeStep={activeStep} setActiveStep={setActiveStep} stepComplete={false} />
                    })}
                </div>
            </section>
        </div>
